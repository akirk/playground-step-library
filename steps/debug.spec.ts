...
describe( 'debug', () => {
	it( 'should set WP_DEBUG to true by default', () => {
		const result = debug( { step: 'debug' }, { steps: [] } );
	...
	it( 'should allow disabling WP_DEBUG', () => {
		const result = debug( { step: 'debug', wpDebug: false }, { steps: [] } );
	...
	it( 'should allow disabling WP_DEBUG_DISPLAY', () => {
		const result = debug( { step: 'debug', wpDebugDisplay: false }, { steps: [] } );
	...
	it( 'should set SCRIPT_DEBUG when enabled', () => {
		const result = debug( { step: 'debug', scriptDebug: true }, { steps: [] } );
	...
	it( 'should allow disabling Query Monitor', () => {
		const result = debug( { step: 'debug', queryMonitor: false }, { steps: [] } );
	...
	it( 'should not install Query Monitor plugin if already present', () => {
		...
		const result = debug( { step: 'debug' }, blueprint );
